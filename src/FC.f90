    ...
    subroutine feature_construction
        ...
        IF (iFCDI > 1 .and. nsis(iFCDI - 1) < nf_sis(iFCDI - 1)) THEN
            if (mpirank == 0) then
                ...
                write (9, '(a)') 'Wrning: Last selected subspace < nf_sis! No more FC will be performed.'
            end if
            return
        end if
        ...
        if (ffdecorr) then
            if (mpisize > 1) then
                ...
                nbasic_select = 2*ceiling(decorr_alpha*nf_sis(iFCDI))
            end if
        else
            if (mpisize > 1) then
        ...
        if (ubound(feat_in1, 2) < i) call addm_in1(i - ubound(feat_in1, 2), feat_in1, name_in1, lastop_in1, complexity_in1, dim_in1)
            ...
            if (mpin(mpirank + 1) > 0) &
                call combine(feat_in1(:, i:j), name_in1(i:j), lastop_in1(i:j), complexity_in1(i:j), dim_in1(:, i:j), &
                             ...
                             trim(adjustl(ops(icomb))), nf(icomb))
        ...
        if (mpirank == 0) then
            ...
            if (ptype == 1) then
                do i = 1, nsis(iFCDI)
                    ...
                    write (fileunit, '(2a,f12.4)') trim(name_sisfeat(i)), '  corr=', tmp_score(1)
                end do
                ...
                do i = 1, nsis(iFCDI)
                    ...
                    if (abs(tmp_score(2)) > 1d9) then
                        write (fileunit, '(2a,i6,a,f12.4)') trim(name_sisfeat(i)), '  N=', nint(tmp_score(1))
                    else
                        ...
                        write (fileunit, '(2a,i6,a,f12.4)') trim(name_sisfeat(i)), '  N=', nint(tmp_score(1)), '  S=', tmp_score(2)
                    end if
                end do
            end if
            close (fileunit)
...
2001        format(*(e20.10))
            if (nreaction == 0) then
                do j = 1, ntask
                    ...
                    if (ptype == 1) then
                        do ll = mm1, mm2
                            write (fileunit, 2001) trainy(ll), sisfeat(ll, :nsis(iFCDI))
                        end do
                    else
                        do ll = mm1, mm2
                            write (fileunit, 2001) sisfeat(ll, :nsis(iFCDI))  ! no y value for categorical properties
                        end do
                    end if
                    close (fileunit)
                end do
            ...
            if (nsis(iFCDI) < nf_sis(iFCDI)) then
                write (*, '(a)') 'Warning: The selected subspace is smaller than nf_sis (SISSO.in) !!!'
            end if
...
124                         continue
    ...
    subroutine sure_indep_screening(nfpcore_this, available, complexity, fname, feat, sisfeat, name_sisfeat)
        ...
        if (mpirank == 0) then
            allocate (score_sisfeat(nf_sis(iFCDI), 2))
            allocate (complexity_sisfeat(nf_sis(iFCDI)))
        end if
